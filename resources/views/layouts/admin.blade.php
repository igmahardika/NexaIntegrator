    ...
    >
        ...
        <nav class="flex-1 px-3.5 py-4 space-y-1 overflow-y-auto">
            ...
            <a href="{{ route('admin.radius.index') }}" class="sidebar-item {{ request()->routeIs('admin.radius*') || request()->routeIs('admin.sites.radius*') ? 'active' : '' }}">
                <div class="w-5 h-5 rounded-md flex items-center justify-center text-xs font-bold {{ request()->routeIs('admin.radius*') || request()->routeIs('admin.sites.radius*') ? 'bg-white/20 text-white' : 'bg-slate-100 text-slate-600' }}">5</div>
                <span class="flex-1">Edge Gateway & RADIUS</span>
                @if($activeSite)
